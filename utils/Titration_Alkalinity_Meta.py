...
class AlkalinityMetadata:
    def __init__(self, filename, filepath, mass, salinity):
        ...
        self.new_meta = pd.DataFrame(  # See https://calkulate.readthedocs.io/en/latest/metadata/
            {"analysis_batch": [np.nan],
             ...
             "file_good": [True],
             # Sets whether total borate is estimated from salinity by PyCO2SYS following: 1 : U74 (default), or 2 : LKB10.
             "opt_total_borate": [self.opt_total_borate],
             # Sets which carbonic acid constants to use from PyCO2SYS, can be any integer from 1 to 16 inclusive. See the PyCO2SYS docs on opt_k_carbonic for details.
             "opt_k_carbonic": [self.opt_k_carbonic],
             # Sets whether the HF dissociation constant is estimated from salinity and temperature by PyCO2SYS following: 1 : DR79 (default), or 2 : PF87.
             "opt_k_fluoride": [self.opt_k_fluoride],
             # Sets whether the bisulfate dissociation constant is estimated from salinity and temperature by PyCO2SYS following: 1 : D90a (default), or 2 : KRCB77.
             "opt_k_bisulfate": [self.opt_k_bisulfate],
    ...
    def Metadata_Export(self):
        ...
        if path.exists(self.filepath/"Alkalinity_Meta.csv"):
            self.new_meta.to_csv(
                self.filepath/"Alkalinity_Meta.csv", mode='a', header=False, index=False)
        else:
            self.new_meta.to_csv(
                self.filepath/"Alkalinity_Meta.csv", mode='a', header=True, index=False)
